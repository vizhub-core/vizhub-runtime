...
export const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.bmp'] as const;
...
const isBase64Encoded = (content: string): boolean => {
  ...
  return /^[A-Za-z0-9+/]*={0,2}$/.test(content.replace(/\s/g, ''));
};
```
...
```typescript
const processHTMLImages = (html: string, files: FileCollection): string => {
  return html.replace(
    /<img[^>]+src=["']([^"']+)["'][^>]*>/gi,
    (match, src) => {
...
<html>
  ...
  "favicon-tiny.jpg": "/9j/4AAQSkZJRgABAQEBLAEsAAD/4RqSRXhpZgAA..." // base64 content
};
```
