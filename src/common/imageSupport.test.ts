...
describe("imageSupport", () => {
  ...
  describe("getImageMimeType", () => {
    it("should return correct MIME types", () => {
      expect(getImageMimeType("photo.jpg")).toBe(
        "image/jpeg",
      );
      expect(getImageMimeType("photo.jpeg")).toBe(
        "image/jpeg",
      );
      expect(getImageMimeType("logo.png")).toBe(
        "image/png",
      );
      expect(getImageMimeType("animation.gif")).toBe(
        "image/gif",
      );
      expect(getImageMimeType("icon.svg")).toBe(
        "image/svg+xml",
      );
      expect(getImageMimeType("image.webp")).toBe(
        "image/webp",
      );
      expect(getImageMimeType("bitmap.bmp")).toBe(
        "image/bmp",
      );
    });
    ...
    it("should return fallback MIME type for unknown extensions", () => {
      expect(getImageMimeType("unknown.xyz")).toBe(
        "image/png",
      );
      expect(getImageMimeType("noextension")).toBe(
        "image/png",
      );
    });
  });
  ...
  describe("convertImageToDataURL", () => {
    it("should convert base64 image to data URL", () => {
      ...
      const result = convertImageToDataURL(
        "test.png",
        base64Content,
      );
      expect(result).toBe(
    ...
    it("should handle GIF files", () => {
      ...
      expect(result).toBe(
        `data:image/gif;base64,${base64Content}`,
      );
    });

    it("should handle WebP files", () => {
      const base64Content =
        "UklGRiQAAABXRUJQVlA4IBgAAAAwAQCdASoBAAABAAA";
      const result = convertImageToDataURL(
        "test.webp",
        base64Content,
      );
    ...
    it("should handle base64-encoded SVG content", () => {
      const base64SVG =
        "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxjaXJjbGUgcj0iMTAiLz48L3N2Zz4=";
      const result = convertImageToDataURL(
        "test.svg",
        base64SVG,
      );
    ...
    it("should handle content with whitespace", () => {
      ...
      expect(result).toBe(
        `data:image/png;base64,${base64WithSpaces}`,
      );
    });
  });
  ...
  describe("processHTMLImages", () => {
    ...
    it("should handle multiple img tags", () => {
      const html =
        '<img src="logo.png" alt="Logo"><img src="icon.jpg" alt="Icon">';
      const files = {
        "logo.png": "PNG_DATA",
        "icon.jpg": "JPG_DATA",
      };
      ...
      expect(result).toContain(
        'src="data:image/jpeg;base64,JPG_DATA"',
      );
    });
    ...
    it("should preserve img tags with non-image file references", () => {
      const html =
        '<img src="script.js" alt="Not an image">';
      const files = {
    ...
    it("should preserve other img attributes", () => {
      const html =
        '<img class="logo" id="main-logo" src="logo.png" alt="Logo" width="100">';
      const files = {
        "logo.png": "PNG_DATA",
      };
      const result = processHTMLImages(html, files);
      expect(result).toBe(
        '<img class="logo" id="main-logo" src="data:image/png;base64,PNG_DATA" alt="Logo" width="100">',
      );
    });
    ...
    it("should handle complex HTML structure", () => {
      ...
      const result = processHTMLImages(html, files);
      expect(result).toContain(
        'src="data:image/jpeg;base64,HEADER_DATA"',
      );
      expect(result).toContain(
        'src="data:image/png;base64,FOOTER_DATA"',
      );
    });
