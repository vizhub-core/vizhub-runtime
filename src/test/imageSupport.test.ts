...
<html>
  ...
  <body>
    ...
    it("should handle empty image files gracefully", async () => {
      ...
      const { html } = await build({ files: emptyImageHTML });
...
container.appendChild(p);
        ...
        "logo.png": "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==",
        "icon.jpg": "/9j/4AAQSkZJRgABAQEBLAEsAAD/4RqSRXhpZgAASUkqAAgAAAAIAA4BAgASAAAAbgAAABIBAwABAAAAAQAAABoBBQABAAAAgAAAABsBBQABAAAAiAAAACgBAwABAAAAAgAAADEBAgANAAAAkAAAADIBAgAUAAAAngAAAGmHBAABAAAAsgAAAOoAAABDcmVhdGVkIHdpdGggR0lNUAAsAQAAAQAAACwBAAABAAAAR0lNUCAyLjEwLjMwAAA"
      };
      ...
      // Should preserve other HTML content
      expect(html).toContain("<h1>V2 Runtime Image Test</h1>");
      expect(html).toContain('alt="Logo"');
      expect(html).toContain('alt="Icon"');

      // Should include bundled JavaScript
      expect(html).toContain('Hello from V2 runtime with images!');
    });
  });
...
export const main = (container) => {
  ...
  \`;
      ...
      expect(html).toContain('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==');
      expect(html).toContain('data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7');
    });
  });
...
container.appendChild(p);
        `,
        "logo.png": "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==",
        "icon.svg": '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="red"/></svg>'
      };
      ...
      expect(html).toContain("<h1>V4 Runtime Image Test</h1>");
